...
- PDF generation currently implemented on the desktop target: one heatmap PDF per Analysis, named `<analysis-name>.pdf`, driven by the `DataStructures.GraphDrawData` a `GenerateAnalysisGraphResponse` carries; and one grid-of-heatmaps PDF per Figure, named `<figure-name>.pdf`, driven by the `DataStructures.FigureDrawData` a `CreateFigureGraphResponse` carries. Both are drawn with the shared `OutMapper.HeatmapDrawing` helper, which takes a `GraphDrawData` directly — `GraphDrawData`/`FigureDrawData` are the validated boundary between UI/TaskManager code and drawing code: they can only be constructed via `Create()`, so `HeatmapDrawing` never re-validates what it's given. `HeatmapDrawing` itself computes no cell/tick/axis geometry inline: it asks `TaskManager.HeatmapLayoutService.ComputeLayout` (a synchronous forwarding call to the pure, Skia-free `Algorithms.HeatmapLayout.Compute`) for a `DataStructures.HeatmapLayoutData`, then does nothing but paint from the coordinates it returns — cell/axis painting is `SKPaint` setup plus `canvas.DrawRect`/`DrawLine` calls, via small `DrawLine`/`DrawRect` helpers that read each `OMLine`/`OMRect`'s own color (and, for lines, width) rather than the caller managing a shared `SKPaint`. Every tick label and axis title is a `DataStructures.OMTextBox` (`Text`, an `OMRect` describing its on-page region, and an `OMTextRotation`); a `DrawTextBox` helper draws each one centered — both horizontally and vertically, via `SKFont.MeasureText`'s ink bounds rather than baseline placement — within its `Rect`, in the color carried on `Rect.FillColor`, rotating around the rect's center for a `CounterClockwise90` box. Font size is not a fixed constant: `OutMapper.TextFitting` computes the largest `SKFont.Size` that fits every tick label (and, separately, both axis titles) to its own `OMTextBox.Rect`, sharing one uniform size per group — a single `SKFont.MeasureText` probe measurement plus exact linear scaling (SkiaSharp glyph metrics scale linearly with font size for a fixed typeface), not an iterative search. `FigureGraphPdfService` similarly asks `TaskManager.HeatmapLayoutService.ComputeReservedMargins` for how much space to reserve around each cell for its own axis chrome, instead of guessing at fixed offsets itself.
...
`TaskManagerService`, `TaskManager.MessageRouter`, `DatasetParsingService`, `CohortParsingService`, `AnalysisService`, `FigureService`, and `HeatmapLayoutService` are all `internal`; `TaskManager.GatewayToOutMapper` is the only *public* entry point, so the message-based boundary with `OutMapper` is enforced by the compiler rather than by convention alone. `HeatmapLayoutService` is a second, narrower crossing point that deliberately bypasses that boundary rather than widening it: `OutMapper` reaches it directly — not through a `Message`/the Gateway — via an `InternalsVisibleTo` grant in `TaskManager.csproj`, since it's a plain, synchronous, side-effect-free forwarding call to `Algorithms.HeatmapLayout.Compute`/`ComputeReservedMargins` (computing one heatmap's cell/tick/axis geometry, or its reserved chrome margins, for `OutMapper.HeatmapDrawing`/`FigureGraphPdfService` to paint from). See [Communication and concurrency](#communication-and-concurrency) for why this bypasses the Gateway instead of using it.
...
- Analysis graph generation requests (`GenerateAnalysisGraphRequest`, carrying a `TwoVariableAnalysisSettings`) and responses (`GenerateAnalysisGraphResponse`, carrying a `DataStructures.GraphDrawData?` on success) — the same validated `GraphDrawData` flows unchanged from `AnalysisService` computation through persistence, the message, and into `OutMapper.HeatmapDrawing`, so drawing code never re-validates.
...
Communication between the UI and task layer is in-process and crosses a single barrier in each direction for *messages*: a **Gateway**. `OutMapper.GatewayToTaskManager` and `TaskManager.GatewayToOutMapper` are the only two classes allowed to send or receive a `Message` across the `OutMapper`/`TaskManager` project boundary; every message enters or leaves a project through its Gateway. Crossing a Gateway is also where the thread switch to or from the UI thread happens — that responsibility belongs to the Gateway, not to `MessageRouter`. One narrow exception to the Gateway rule exists for non-message calls — see [A second, non-message crossing point](#a-second-non-message-crossing-point-heatmaplayoutservice) below.
...
### A second, non-message crossing point: `HeatmapLayoutService`

Not every cross-boundary call is a `Message` through a Gateway. `TaskManager.HeatmapLayoutService` is `internal`, reachable from `OutMapper` via an `InternalsVisibleTo` grant in `TaskManager.csproj` rather than a project reference to `Algorithms` — a plain synchronous forwarding call to `Algorithms.HeatmapLayout.Compute`/`ComputeReservedMargins`, invoked directly by `OutMapper.HeatmapDrawing`/`FigureGraphPdfService` while painting a PDF page.

This is a deliberate, narrow exception, not a precedent for bypassing the Gateway more broadly. The `Message`/Gateway pipeline exists to serialize asynchronous, potentially I/O-bound work off the UI thread and marshal its result back via `DispatcherQueue.TryEnqueue` — the right tool for the disk-touching work described above, but the wrong one for a pure, side-effect-free geometry calculation that PDF rendering needs an answer to immediately, on the thread already doing the drawing. An earlier attempt to route this same computation through the `Message`/`GatewayToOutMapper` pipeline instead was reverted after it was found to hang indefinitely under test: that path depends on a live UI `DispatcherQueue`, captured by `GatewayToTaskManager.Initialize()` (called once from `App.OnLaunched`), which no test host provides — so the response message was silently never delivered, and any caller awaiting it blocked forever.
...
- Creating a Figure's graph (`FigureService.CreateGraph`) persists the request's row/column counts and cell assignments as the figure's authoritative `figure-config.json`, then for each cell reads the assigned Analysis's `graph-data.json` via `AnalysisService.ReadPersistedGraphData`. A cell with no assignment, or whose Analysis has no persisted graph data (never successfully generated, or later deleted), is recorded as `false` in the row-major `CellHasGraph` mask and contributes nothing to `Graphs` — the Figure is still produced, with that cell left blank in `OutMapper.FigureGraphPdfService`'s drawing. The assembled mask and graphs are validated together into one `DataStructures.FigureDrawData` (`FigureDrawData.Create`), which becomes the `CreateFigureGraphResponse`'s payload.
- The Figure PDF (`<figure-name>.pdf`, one per Figure) is drawn by `OutMapper.FigureGraphPdfService`, laying out one small heatmap per assigned cell, with a gap between cells (scaled to fit the figure's row/column grid within a single page) using the same `OutMapper.HeatmapDrawing` helper `AnalysisGraphPdfService` uses, and written into the selected project's `OutMapper_ProjectOutput` directory. The Figure's name is not drawn on the page. If the figure's `FigureLabelStyle` is not `None`, each assigned cell also gets a letter label (row-major order, skipping empty cells), positioned at the cell's top-right corner to avoid the axis chrome `HeatmapDrawing` draws along a cell's left/bottom edges; each cell also reserves space for its own axis tick labels/titles within its own bounds (via `TaskManager.HeatmapLayoutService.ComputeReservedMargins`, sharing the same offset constants `Algorithms.HeatmapLayout.Compute` itself uses so the two can't drift apart), so that chrome can't bleed into a neighboring cell.
...
PDF drawing logic is tested independently of file writing: `HeatmapDrawing.Draw` takes a plain `SKCanvas` and data, with no I/O, so it's tested by drawing onto an in-memory `SKSurface` and asserting on pixel colors. The underlying cell/tick/axis geometry itself (the row-zero-at-bottom flip, tick positions and text formatting, axis-title placement, reserved margins) is tested one layer below that, entirely Skia-free, in `Algorithms.Tests`/`DataStructures.Tests` (`HeatmapLayoutTests`, `HeatmapLayoutDataTests`) — `HeatmapDrawing`'s own tests only need to cover the painting step, since the geometry it paints from is already covered where it's computed. The auto-fit font-sizing math itself (`OutMapper.TextFitting`) is pure logic given a typeface/text/width/height budget, so it's unit tested directly without a live `SKSurface`. The PDF services' *file-writing* behavior (right path, non-empty `%PDF`-prefixed output) is tested separately, against `InMemoryFileSystem`. For visual inspection of a generated PDF's actual appearance — useful while the layout is still being iterated on, before a snapshot/approval-based regression system would be worth the cost — `Tests/OutMapper.Tests/SamplePdfGenerator.cs` and `SampleFigurePdfGenerator.cs` render one representative PDF each using the real `LocalFileSystem`. They run as regular `[Fact]`s (fast enough not to warrant skipping) and write into their own subfolder under `OutMapperAutomatedTestsOutput/` at the solution root, via `TestSupport.SampleOutputDirectory.For(nameof(YourTestClass))` — this keeps concurrent generators from colliding on the same output file. That folder is gitignored and allow-listed for reads in `.claude/settings.json`, so the generated PDF can be read directly afterward (Claude's `Read` tool renders PDF pages) without a permission prompt. Any new sample-output generator should follow the same `SampleOutputDirectory` pattern.
...
- `HeatmapDrawing`'s tick labels and axis titles auto-fit their font size to their region via `OutMapper.TextFitting`, but `AnalysisGraphPdfService`'s page title and `FigureGraphPdfService`'s cell letter labels still use a hardcoded `SKFont.Size` — a deliberate scope decision when `TextFitting` was introduced, not yet extended to those two call sites.
